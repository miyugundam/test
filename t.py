...
async def fetch_peers_for_delete(update: Update, context: CallbackContext):
    """Fetch peers for the selected interface and display them."""
    query = update.callback_query
    await query.answer()
    ...
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"delete_peer_{peer['peer_name']}")] for peer in peers]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.message.reply_text("Select a peer to delete:", reply_markup=reply_markup)
...
def main():
    ...
    states={
        ...
        SELECT_PEER_FOR_DELETE: [CallbackQueryHandler(confirm_delete_peer, pattern="delete_peer_.*")],
