...
async def collect_interface(update: Update, context: CallbackContext):
    ...
    private_ip_message = (
        ...
        "\n\nPlease select an IP or provide a custom IP:"
    )

    await update.message.reply_text(private_ip_message, parse_mode="Markdown")
...
async def collect_data_limit(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        ...
        "Is this the **first usage** of the peer? (yes/no):"
    )
    return "COLLECT_FIRST_USAGE"

async def collect_first_usage(update: Update, context: CallbackContext):
    """Collect whether it's the first usage of the peer."""
    first_usage = update.message.text.lower()
    if first_usage not in ["yes", "no"]:
        await update.message.reply_text("❌ Invalid input. Please answer with 'yes' or 'no':")
        return "COLLECT_FIRST_USAGE"

    context.user_data["first_usage"] = first_usage == "yes"
    await update.message.reply_text(
        f"✅ **First Usage:** {'Yes' if context.user_data['first_usage'] else 'No'}\n\n"
