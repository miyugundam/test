...
SELECT_INTERFACE, SELECT_IP, PEER_DETAILS, DATA_LIMIT, DNS, EXPIRY_TIME, SET_FIRST_USAGE = range(7)
...
async def collect_peer_details(update: Update, context: CallbackContext):
    """Collect peer name and proceed to data limit."""
    ...
    await update.message.reply_text(
        "Enter the **data limit** for the peer (e.g., `500MiB` or `1GiB`):",
        parse_mode="Markdown"
    )
    return DATA_LIMIT
...
async def set_data_limit(update: Update, context: CallbackContext):
    """Validate and store the data limit."""
    ...
    await update.message.reply_text(
        "Enter the **DNS** (e.g., `1.1.1.1` or `8.8.8.8`). You can enter multiple DNS servers separated by commas:",
        parse_mode="Markdown"
    )
    return DNS
...
async def set_dns(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        "Enter the **expiry time** in the format:\n"
        "`days, months, hours, minutes`\n"
        "For example: `10, 0, 0, 0` (10 days, 0 months, 0 hours, 0 minutes).",
        parse_mode="Markdown"
    )
    return EXPIRY_TIME
...
async def set_expiry_time(update: Update, context: CallbackContext):
    """Validate and store the expiry time."""
    try:
        days, months, hours, minutes = map(int, update.message.text.split(","))
        if any(x < 0 for x in [days, months, hours, minutes]):
            raise ValueError("Expiry time values cannot be negative.")
        if days == 0 and months == 0 and hours == 0 and minutes == 0:
            raise ValueError("Total expiry time must be greater than zero.")
        ...
        await update.message.reply_text(f"❌ Invalid expiry time format: {e}")
        return EXPIRY_TIME
...
def main():
    ...
    states={
        ...
        DATA_LIMIT: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_data_limit)],
        DNS: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_dns)],
