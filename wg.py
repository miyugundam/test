...
def main():
    ...
    # For handling download peer configuration
    application.add_handler(CallbackQueryHandler(handle_download_peer_config, pattern="^download_peer_config$"))

# For handling generate QR code
    application.add_handler(CallbackQueryHandler(handle_generate_peer_qr, pattern="^generate_peer_qr$"))

# Handle specific commands/messages for peer config download
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(r'^\w+\.conf$'), send_peer_config))

# Handle specific commands/messages for peer QR generation
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND & filters.Regex(r'^\w+\s\w+\.conf$'), send_peer_qr))
