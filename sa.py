...
TOGGLE_BLOCK = range(1)  # [0]
SELECT_CONFIG, SELECT_PEER, TOGGLE_BLOCK = range(1, 4)  # [1, 2, 3]
SHOW_BACKUPS, CREATE_BACKUP, DELETE_BACKUP, RESTORE_BACKUP = range(4, 8)  # [4, 5, 6, 7]
SELECT_INTERFACE, PEER_NAME, PEER_IP, DATA_LIMIT, CONFIG_FILE, FIRST_USAGE, CONFIRM_PEER = range(7)
...
async def collect_peer_ip(update: Update, context: CallbackContext):
    ...
    if update.callback_query:
        ...
        context.user_data["peer_ip"] = peer_ip  # Save the selected IP
        await query.message.reply_text(
            f"✅ **Peer IP:** {peer_ip}\n\n"
        ...
        # Save IP in user data and proceed
        ...
        logging.error("Unexpected update type: No callback query or message found.")
        return ConversationHandler.END
...
async def collect_first_usage(update: Update, context: CallbackContext):
    ...
    await update.message.reply_text(
        ...
        "Enter **Persistent Keepalive** value (default: 25 seconds, optional):",
        parse_mode="Markdown",
    )
    return CONFIG_FILE
...
async def collect_config_file(update: Update, context: CallbackContext):
    """Collect the persistent keepalive and config file name."""
    ...
    # Optionally collect mtu
    await update.message.reply_text(
        "Enter **MTU** value (default: 1280, optional):",
        parse_mode="Markdown"
    )
    return CONFIRM_PEER
...
def register_peer_handlers(application):
    ...
    states={
        ...
        "COLLECT_PEER_IP": [
            CallbackQueryHandler(collect_peer_ip, pattern=r"select_ip_.*"),  # Handle IP selection via buttons
            MessageHandler(filters.TEXT & ~filters.COMMAND, collect_peer_ip),  # Handle custom IP input
        ],
...
def main():
    ...
    # Corrected ConversationHandler for Peer Creation
    peer_creation_conversation = ConversationHandler(
    entry_points=[CallbackQueryHandler(create_peer, pattern="create_peer")],
    states={
        SELECT_INTERFACE: [MessageHandler(filters.TEXT & ~filters.COMMAND, collect_interface)],
        PEER_IP: [
            CallbackQueryHandler(collect_peer_ip, pattern=r"select_ip_.*"),  # Handle IP selection via buttons
            MessageHandler(filters.TEXT & ~filters.COMMAND, collect_peer_ip),  # Handle custom IP input
        ],
        ...
        FIRST_USAGE: [MessageHandler(filters.TEXT & ~filters.COMMAND, collect_first_usage)],  # Correctly reference FIRST_USAGE
        CONFIG_FILE: [MessageHandler(filters.TEXT & ~filters.COMMAND, collect_config_file)],
        CONFIRM_PEER: [MessageHandler(filters.TEXT & ~filters.COMMAND, confirm_peer_creation)],
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
)
