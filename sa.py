...
SELECT_INTERFACE, SELECT_IP, PEER_DETAILS, SET_FIRST_USAGE = range(4)
...
async def set_first_usage(update: Update, context: CallbackContext):
    ...
    # Fetch the selected configuration file
    selected_interface = context.user_data.get("selected_interface")
    if not selected_interface or not re.match(r"^[a-zA-Z0-9_-]+\.conf$", selected_interface):
        await query.message.reply_text("❌ Invalid or missing configuration file name.")
        return ConversationHandler.END
    ...
    payload = {
        ...
        "dataLimit": "500MiB",  # Replace with user input if needed
        "configFile": selected_interface,  # Use the correctly set config file
        "firstUsage": first_usage,
        "dns": "1.1.1.1",  # Replace with user input if needed
        "expiryDays": 0,  # Replace with user input
        "expiryMonths": 0,  # Replace with user input
        "expiryHours": 0,  # Replace with user input
        "expiryMinutes": 0,  # Replace with user input
    }
