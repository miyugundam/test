...
async def create_peer(update: Update, context: CallbackContext):
    """Display a list of available WireGuard interfaces to start creating a peer."""
    query = update.callback_query
    await query.answer()

    # Fetch available WireGuard interfaces dynamically
    response = await api_request("api/get-interfaces")  # Replace with your endpoint to list interfaces
    if "error" in response:
    ...
    if not interfaces:
        await query.message.reply_text("No WireGuard interfaces found. Please configure at least one.")
        return ConversationHandler.END

    # Display interfaces as inline buttons
...
async def collect_peer_name(update: Update, context: CallbackContext):
    ...
    # Extract selected interface from callback data
