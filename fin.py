...
SELECT_INTERFACE_CREATION, SELECT_IP_CREATION, PEER_DETAILS_CREATION, DATA_LIMIT_UNIT_CREATION, DATA_LIMIT_VALUE_CREATION, DNS_CREATION, DNS_CUSTOM_CREATION, EXPIRY_TIME_CREATION, SET_FIRST_USAGE_CREATION = range(100, 109)
...
async def create_peer_start(update: Update, context: CallbackContext):
    ...
    print("create_peer_start triggered")  # Debugging
    chat_id = update.effective_chat.id

    response = await api_request("api/get-interfaces")
    print(f"Response from get-interfaces API: {response}")  # Debugging
    ...
    # Log button callback data
    keyboard = [[InlineKeyboardButton(interface, callback_data=f"select_interface_{interface}")] for interface in interfaces]
    print(f"Generated buttons: {keyboard}")  # Debugging

    reply_markup = InlineKeyboardMarkup(keyboard)
    await context.bot.send_message(chat_id, text="Select a WireGuard interface:", reply_markup=reply_markup)

    print("Transitioning to SELECT_INTERFACE_CREATION state")  # Debugging
    return SELECT_INTERFACE_CREATION
...
async def select_interface(update: Update, context: CallbackContext):
    ...
    print(f"Callback data received in select_interface: {query.data}")  # Debugging
    selected_interface = query.data.replace("select_interface_", "") + ".conf"
    context.user_data["selected_interface"] = selected_interface
    print(f"Selected interface: {selected_interface}")  # Debugging

    # Fetch available IPs
    response = await api_request(f"api/available-ips?interface={selected_interface.replace('.conf', '')}")
    print(f"Available IPs API response: {response}")  # Debugging
    ...
    print("Transitioning to SELECT_IP_CREATION state")  # Debugging
    return SELECT_IP_CREATION
...
async def collect_peer_details(update: Update, context: CallbackContext):
    ...
    return DATA_LIMIT_UNIT_CREATION
...
async def set_data_limit_unit(update: Update, context: CallbackContext):
    ...
    return DATA_LIMIT_VALUE_CREATION
...
async def set_expiry_time(update: Update, context: CallbackContext):
    ...
    return SET_FIRST_USAGE_CREATION
...
def main():
    ...
    states={
        SELECT_INTERFACE_CREATION: [CallbackQueryHandler(select_interface, pattern="select_interface_.*")],
        SELECT_IP_CREATION: [CallbackQueryHandler(select_ip, pattern="select_ip_.*")],
        PEER_DETAILS_CREATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, collect_peer_details)],
        DATA_LIMIT_UNIT_CREATION: [CallbackQueryHandler(set_data_limit_unit, pattern="unit_.*")],
        DATA_LIMIT_VALUE_CREATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_data_limit_value)],
        DNS_CREATION: [CallbackQueryHandler(set_dns, pattern="dns_.*")],
        DNS_CUSTOM_CREATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_custom_dns)],
        EXPIRY_TIME_CREATION: [MessageHandler(filters.TEXT & ~filters.COMMAND, set_expiry_time)],
        SET_FIRST_USAGE_CREATION: [CallbackQueryHandler(set_first_usage, pattern="first_usage_.*")],
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
    allow_reentry=True,
)
