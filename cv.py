...
def register_backup_handlers(application):
    ...
    application.add_handler(CallbackQueryHandler(delete_backup, pattern="delete_backup_.*"))  # More specific
    application.add_handler(CallbackQueryHandler(restore_backup, pattern="restore_backup_.*"))  # More specific
...
async def delete_peer_menu(update: Update, context: CallbackContext):
    """Prompt the user to enter the peer name to search."""
    chat_id = update.effective_chat.id
...
async def handle_peer_name_input(update: Update, context: CallbackContext):
    """Store the peer name and prompt for WireGuard interface selection."""
    ...
    keyboard = [[InlineKeyboardButton(interface, callback_data=f"select_interface_{interface}")]
                for interface in interfaces]
    keyboard.append([InlineKeyboardButton("🔙 Back to Peer Menu", callback_data="delete_peer_menu")])
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_interface_selection(update: Update, context: CallbackContext):
    ...
    peer_name = context.user_data.get("search_peer_name")
    ...
    # Show filtered peer details with delete button underneath
    for peer in peers:
        peer_details = (
            f"🔷 **Peer Details**\n"
            ...
            f"⚡ **Status:** {'🟢 Active' if not peer.get('blocked', False) else '🔴 Blocked'}"
        )
        context.user_data["selected_peer_name"] = peer['peer_name']  # Store peer name in user data
        context.user_data["selected_peer"] = peer  # Store the entire peer data for deletion
        keyboard = [
            [InlineKeyboardButton("❌ Delete Peer", callback_data=f"delete_peer_{peer['peer_name']}")],
            [InlineKeyboardButton("🔙 Back to Peer Menu", callback_data="delete_peer_menu")]
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_peer_deletion(update: Update, context: CallbackContext):
    ...
    callback_data = query.data.replace("delete_peer_", "")
    selected_peer = context.user_data.get("selected_peer")

    if not selected_peer or selected_peer["peer_name"] != callback_data:
    ...
    payload = {"peerName": selected_peer["peer_name"], "configFile": config_file}
    ...
    if "error" in response:
        ...
        message = response.get("message", f"Peer `{selected_peer['peer_name']}` deleted successfully.")
        await query.message.reply_text(f"✅ {message}")

    # Add a back button to return to the peer menu
    keyboard = [[InlineKeyboardButton("🔙 Back to Peer Menu", callback_data="delete_peer_menu")]]
    reply_markup = InlineKeyboardMarkup(keyboard)
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(delete_peer_menu, pattern="delete_peeer"))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_peer_name_input))
    application.add_handler(CallbackQueryHandler(handle_interface_selection, pattern="select_interface_.*"))
    application.add_handler(CallbackQueryHandler(handle_peer_deletion, pattern="delete_selected_peer"))
