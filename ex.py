...
async def send_peer_config(update: Update, context: CallbackContext):
    peer_name, config_name = update.message.text.split()
    url = f"{API_BASE_URL}/api/export-peer-config?peerName={peer_name}&config={config_name}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers={"Authorization": f"Bearer {API_KEY}"}) as response:
            if response.status == 200:
                config_data = await response.text()
                await update.message.reply_document(
                    document=BytesIO(config_data.encode()),
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_generate_peer_qr, pattern="generate_peer_qr"))
    application.add_handler(CallbackQueryHandler(handle_download_peer_config, pattern="download_peer_config"))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, send_peer_qr))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, send_peer_config))
