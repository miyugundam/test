...
async def create_peer(update: Update, context: CallbackContext):
    ...
    # Fetch available WireGuard interfaces
    ...
    # Create buttons for interfaces
    keyboard = [[InlineKeyboardButton(interface, callback_data=f"interface_select_{interface}")] for interface in interfaces]
...
async def handle_interface_selection(update: Update, context: CallbackContext):
    """Handle the selection of a WireGuard interface."""
    query = update.callback_query
    await query.answer()

    # Extract and store the selected interface
    selected_interface = query.data.replace("interface_select_", "")
    context.user_data["config_name"] = selected_interface
...
# Step 2: Collect Peer Name After Selecting Interface
async def collect_peer_name(update: Update, context: CallbackContext):
    """Handle interface selection and prompt for peer name."""
    query = update.callback_query
    await query.answer()
    logging.info(f"Callback data received: {query.data}")

    # Extract selected interface and store it in user data
    selected_interface = query.data.replace("select_interface_", "")
    logging.info(f"Extracted interface: {selected_interface}")
    context.user_data["config_name"] = selected_interface

    # Check if this is create or edit flow
    if context.user_data.get("edit_mode"):
        ...
        await query.message.reply_text(
            f"📛 **Selected Interface:** {selected_interface}\n\n"
            "Enter the name of the new peer (letters, numbers, and underscores only):",
            parse_mode="Markdown"
        )
        return PEER_NAME
...
async def edit_peer(update: Update, context: CallbackContext):
    ...
    # Create buttons for interfaces
    keyboard = [[InlineKeyboardButton(interface, callback_data=f"interface_edit_{interface}")] for interface in interfaces]
    keyboard.append([InlineKeyboardButton("🔙 Back to Peers Menu", callback_data="peers_menu")])
    reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_edit_interface(update: Update, context: CallbackContext):
    """Handle interface selection for editing."""
    query = update.callback_query
    await query.answer()

    # Extract and store the selected interface
    selected_interface = query.data.replace("interface_edit_", "")
    context.user_data["config_name"] = selected_interface
...
def main():
    ...
    states={
        SELECT_INTERFACE: [CallbackQueryHandler(handle_interface_selection, pattern="interface_select_.*")],
    ...
    states={
        SELECT_INTERFACE: [CallbackQueryHandler(handle_edit_interface, pattern="interface_edit_.*")],
