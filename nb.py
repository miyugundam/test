...
async def peers_menu(update: Update, context: CallbackContext):
    """Display the peer management menu."""
    chat_id = update.effective_chat.id
    message = (
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 Reset Traffic", callback_data="reset_traffic_menu")],
        [InlineKeyboardButton("⏳ Reset Expiry", callback_data="reset_expiry_menu")],
...
async def reset_menu_start(update: Update, context: CallbackContext):
    """Start the reset menu process by selecting a WireGuard interface."""
    chat_id = update.effective_chat.id
    ...
    # Create buttons for each interface
    keyboard = [[InlineKeyboardButton(interface, callback_data=f"reset_select_interface_{interface}")] for interface in interfaces]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await context.bot.send_message(chat_id, text="Select a WireGuard interface:", reply_markup=reply_markup)
    return SELECT_INTERFACE


async def reset_select_interface(update: Update, context: CallbackContext):
    """Handle WireGuard interface selection for resetting peer traffic or expiry."""
    query = update.callback_query
    await query.answer()

    # Store selected interface
    selected_interface = query.data.replace("reset_select_interface_", "")
    context.user_data["selected_interface"] = selected_interface
    ...
    # Display the first 5 peers and provide a search option
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"reset_{peer['peer_name']}")] for peer in peers[:5]]
    keyboard.append([InlineKeyboardButton("🔍 Search for Peer", callback_data="reset_search_peer")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.message.reply_text("Select a peer to reset traffic or expiry:", reply_markup=reply_markup)
    return SELECT_PEER


async def reset_search_peer(update: Update, context: CallbackContext):
    """Prompt the user to search for a peer by name."""
    await update.callback_query.answer()
    await update.callback_query.message.reply_text(
        "Enter the **peer name** to search for:",
        parse_mode="Markdown"
    )
    return SEARCH_PEER


async def handle_peer_search(update: Update, context: CallbackContext):
    """Search and display peers matching the entered name."""
    peer_name = update.message.text.strip()
    selected_interface = context.user_data.get("selected_interface")
    ...
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"reset_{peer['peer_name']}")] for peer in matched_peers]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Select a peer to reset traffic or expiry:", reply_markup=reply_markup)
    return SELECT_PEER


async def select_peer_for_reset(update: Update, context: CallbackContext):
    """Handle peer selection and provide reset options."""
    query = update.callback_query
    await query.answer()

    selected_peer = query.data.replace("reset_", "")
    context.user_data["selected_peer"] = selected_peer

    # Display current peer details for reference
    selected_interface = context.user_data["selected_interface"]
    response = await api_request(f"api/peers-by-interface?interface={selected_interface}")
    peers = response.get("peers", [])
    peer_data = next((peer for peer in peers if peer["peer_name"] == selected_peer), None)
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔙 Back to Peer List", callback_data="reset_select_interface")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    ...
    await query.message.reply_text(
        f"🔷 **Current Peer Details** 🔷\n"
        f"━━━━━━━━━━━━━━━━━━━━━━━\n"
        f"📛 **Peer Name:** `{peer_data['peer_name']}`\n"
...
async def reset_traffic(update: Update, context: CallbackContext):
    ...
    payload = {"peerName": peer_name, "config": f"{selected_interface}.conf"}
    response = await api_request("api/reset-traffic", method="POST", data=payload)

    if "error" in response:
        await query.message.reply_text(f"❌ Error resetting traffic: {response['error']}")
    else:
        await query.message.reply_text(f"✅ Traffic for peer '{peer_name}' has been reset successfully!")
    return ConversationHandler.END
...
async def reset_expiry(update: Update, context: CallbackContext):
    ...
    payload = {"peerName": peer_name, "config": f"{selected_interface}.conf"}
    response = await api_request("api/reset-expiry", method="POST", data=payload)

    if "error" in response:
        await query.message.reply_text(f"❌ Error resetting expiry: {response['error']}")
    else:
        await query.message.reply_text(f"✅ Expiry for peer '{peer_name}' has been reset successfully!")
    return ConversationHandler.END
...
def main():
    ...
    peer_management_conversation = ConversationHandler(
    entry_points=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
    states={
        SELECT_INTERFACE: [
            CallbackQueryHandler(edit_select_interface, pattern="edit_select_interface_.*"),
            CallbackQueryHandler(reset_select_interface, pattern="reset_select_interface_.*"),
        ],
        SEARCH_PEER: [MessageHandler(filters.TEXT & ~filters.COMMAND, handle_peer_search)],
        SELECT_PEER: [
            CallbackQueryHandler(select_peer_for_reset, pattern="reset_.*"),
        ],
        RESET_OPTION: [
            CallbackQueryHandler(reset_traffic, pattern="reset_traffic"),
            CallbackQueryHandler(reset_expiry, pattern="reset_expiry"),
        ],
        # Include other states such as create_peer, edit_peer, etc.
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
)
    ...
    application.add_handler(peer_management_conversation)
