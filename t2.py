...
SELECT_INTERFACE_FOR_DELETE, SELECT_PEER_FOR_DELETE, CONFIRM_DELETE_PEER = range(3)
...
def register_backup_handlers(application):
    ...
    application.add_handler(CallbackQueryHandler(delete_backup, pattern="delete_backup.*"))
...
async def delete_peer_start(update: Update, context: CallbackContext):
    """Prompt the user to select a WireGuard interface for deletion."""
    chat_id = update.effective_chat.id
    ...
    keyboard = [[InlineKeyboardButton(interface, callback_data=f"delete_interface_{interface}")] for interface in interfaces]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await context.bot.send_message(chat_id, text="Select a WireGuard interface:", reply_markup=reply_markup)
    return SELECT_INTERFACE_FOR_DELETE



async def fetch_peers_for_delete(update: Update, context: CallbackContext):
    """Fetch a batch of peers for the selected interface and display them."""
    query = update.callback_query
    await query.answer()

    # Get the selected interface (set it only once)
    if "delete_interface_" in query.data:
        selected_interface = query.data.replace("delete_interface_", "")
        context.user_data["selected_interface"] = selected_interface

    selected_interface = context.user_data["selected_interface"]

    # Fetch the first batch of peers
    response = await api_request(f"api/peers-by-interface?interface={selected_interface}&limit=5&page=1")
    if "error" in response:
    ...
    # Save peers and pagination state
    context.user_data["peers"] = peers
    context.user_data["peer_page"] = 1

    # Display the peers
    keyboard = [
        [InlineKeyboardButton(peer["peer_name"], callback_data=f"delete_peer_{peer['peer_name']}")]
        for peer in peers
    ]
    keyboard.append([InlineKeyboardButton("🔍 Search Peer by Name", callback_data="search_peer_name")])
    keyboard.append([InlineKeyboardButton("➡️ Next Batch", callback_data="next_peer_batch")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.message.reply_text("Select a peer to delete or choose an action:", reply_markup=reply_markup)
    return SELECT_PEER_FOR_DELETE  # Transition to the correct state



async def fetch_next_peer_batch(update: Update, context: CallbackContext):
    """Fetch the next batch of peers and display them."""
    query = update.callback_query
    await query.answer()

    selected_interface = context.user_data["selected_interface"]
    current_page = context.user_data.get("peer_page", 1)
    next_page = current_page + 1

    # Fetch the next batch
    response = await api_request(f"api/peers-by-interface?interface={selected_interface}&limit=5&page={next_page}")
    if "error" in response:
        await query.message.reply_text(f"❌ Error fetching peers: {response['error']}")
        return SELECT_PEER_FOR_DELETE

    peers = response.get("peers", [])
    if not peers:
        await query.message.reply_text("No more peers to display.")
        return SELECT_PEER_FOR_DELETE

    # Update pagination and display the next batch
    context.user_data["peer_page"] = next_page
    keyboard = [
        [InlineKeyboardButton(peer["peer_name"], callback_data=f"delete_peer_{peer['peer_name']}")]
        for peer in peers
    ]
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.message.reply_text("Select a peer to delete or choose an action:", reply_markup=reply_markup)
    return SELECT_PEER_FOR_DELETE  # Ensure we stay in the correct state



async def fetch_peers_by_name(update: Update, context: CallbackContext):
    """Fetch peers matching the user's input name."""
    ...
    # Display the matched peers
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"delete_peer_{peer['peer_name']}")] for peer in matched_peers]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await update.message.reply_text("Select a peer to delete:", reply_markup=reply_markup)
    return SELECT_PEER_FOR_DELETE

async def confirm_delete_peer(update: Update, context: CallbackContext):
    """Show peer details and prompt for deletion confirmation."""
    query = update.callback_query
    await query.answer()

    peer_name = query.data.replace("delete_peer_", "")
    context.user_data["peer_name"] = peer_name
    selected_interface = context.user_data["selected_interface"]

    response = await api_request(f"api/peers-by-interface?interface={selected_interface}")
    peers = response.get("peers", [])
    matched_peers = [peer for peer in peers if peer["peer_name"] == peer_name]

    if not matched_peers:
        await query.message.reply_text(f"No peer found with name: **{peer_name}**.")
        return SELECT_PEER_FOR_DELETE
    ...
    for peer in matched_peers:
        ...
        keyboard = [[InlineKeyboardButton("❌ Delete", callback_data="confirm_delete")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
        await query.message.reply_text(peer_details, reply_markup=reply_markup, parse_mode="Markdown")
    return CONFIRM_DELETE_PEER



async def delete_peer(update: Update, context: CallbackContext):
    ...
    response = await api_request(
        "api/delete-peer",
        method="DELETE",
        data={"peerName": peer_name, "config": selected_interface}
    )

    if "error" in response:
        await query.message.reply_text(f"❌ Error deleting peer: {response['error']}")
    else:
        await query.message.reply_text(f"✅ Peer `{peer_name}` deleted successfully!")
...
def main():
    ...
    entry_points=[CallbackQueryHandler(delete_peer_start, pattern="delete_peer")],
    states={
        SELECT_INTERFACE_FOR_DELETE: [CallbackQueryHandler(fetch_peers_for_delete, pattern="delete_interface_.*")],
        SELECT_PEER_FOR_DELETE: [
            CallbackQueryHandler(fetch_next_peer_batch, pattern="next_peer_batch"),
            CallbackQueryHandler(search_peer_by_name, pattern="search_peer_name"),
            CallbackQueryHandler(confirm_delete_peer, pattern="delete_peer_.*"),
        ],
        SEARCH_PEER: [MessageHandler(filters.TEXT & ~filters.COMMAND, fetch_peers_by_name)],
        CONFIRM_DELETE_PEER: [CallbackQueryHandler(delete_peer, pattern="confirm_delete")],
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
    allow_reentry=True,
)
