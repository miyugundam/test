...
SELECT_INTERFACE, SELECT_PEER_OR_SEARCH, SEARCH_PEER, SELECT_PEER_TO_EDIT, EDIT_OPTION, SET_DATA_LIMIT, SET_DNS, SET_EXPIRY_TIME = range(7)
...
async def peers_menu(update: Update, context: CallbackContext):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("🔄 Reset Traffic", callback_data="reset_traffic")],  # New button
        [InlineKeyboardButton("⏳ Reset Expiry", callback_data="reset_expiry")],    # New button
        [InlineKeyboardButton("🔙 Back to Main Menu", callback_data="main_menu")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await context.bot.send_message(chat_id=chat_id, text=message, reply_markup=reply_markup, parse_mode="Markdown")

async def reset_traffic_prompt(update: Update, context: CallbackContext):
    """Prompt the user to select a peer for resetting traffic."""
    query = update.callback_query
    await query.answer()

    # Reset state and fetch peers
    context.user_data["peers"] = []
    context.user_data["peer_page"] = 0
    ...
    # Save peers in context for pagination
    context.user_data["peers"] = peers

    # Show the first page
    await show_peer_page_traffic(update, context)
    return SELECT_PEER


async def show_peer_page_traffic(update: Update, context: CallbackContext):
    """Show a paginated list of peers for Reset Traffic."""
    peers = context.user_data.get("peers", [])
    page = context.user_data.get("peer_page", 0)
    page_size = 5
    start = page * page_size
    end = start + page_size

    # Create buttons for the current page
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"reset_traffic_{peer['peer_name']}")] for peer in peers[start:end]]

    # Add navigation buttons
    if start > 0:
        keyboard.append([InlineKeyboardButton("⬅️ Previous", callback_data="peer_page_previous_traffic")])
    if end < len(peers):
        keyboard.append([InlineKeyboardButton("➡️ Next", callback_data="peer_page_next_traffic")])

    # Add a search option and back button
    keyboard.append([InlineKeyboardButton("🔍 Search Peer by Name", callback_data="search_peer_name_traffic")])
    keyboard.append([InlineKeyboardButton("🔙 Back to Peers Menu", callback_data="peers_menu")])

    reply_markup = InlineKeyboardMarkup(keyboard)

    # Retrieve the current query message text and compare with the new message
    query = update.callback_query
    current_message = query.message.text if query and query.message else ""
    new_message = "Select a peer to reset traffic:"

    # Only edit the message if the content or markup has changed
    if current_message != new_message or query.message.reply_markup != reply_markup:
        await query.edit_message_text(new_message, reply_markup=reply_markup)


async def handle_peer_pagination_traffic(update: Update, context: CallbackContext):
    """Handle pagination buttons for Reset Traffic."""
    query = update.callback_query
    await query.answer()

    if query.data == "peer_page_previous_traffic":
        context.user_data["peer_page"] -= 1
    elif query.data == "peer_page_next_traffic":
        context.user_data["peer_page"] += 1

    # Call the function to display the updated page
    await show_peer_page_traffic(update, context)



async def search_peer_by_name_traffic(update: Update, context: CallbackContext):
    """Prompt the user to enter a peer name for Reset Traffic."""
    query = update.callback_query
    await query.answer()

    await query.message.reply_text(
        "Enter the **name of the peer** to search for (case-insensitive):",
        parse_mode="Markdown"
    )
    return SEARCH_PEER


async def filter_peers_by_name_traffic(update: Update, context: CallbackContext):
    """Filter peers for Reset Traffic by the user's input."""
    peer_name = update.message.text.strip()
    selected_interface = context.user_data.get("selected_interface")  # Retrieve the selected interface

    if not selected_interface:
        await update.message.reply_text("❌ No interface selected. Please go back and select an interface.")
        return ConversationHandler.END
    ...
    peers = response.get("peers", [])
    # Filter peers by name (case-insensitive)
    matched_peers = [peer for peer in peers if peer_name.lower() in peer["peer_name"].lower()]

    if not matched_peers:
        await update.message.reply_text(f"No peers found matching: **{peer_name}**.", parse_mode="Markdown")
        return SEARCH_PEER

    # Update pagination state with the filtered peers
    context.user_data["peers"] = matched_peers
    context.user_data["peer_page"] = 0

    # Show the first page of matched peers
    await show_peer_page_traffic(update, context)
    return SELECT_PEER



async def reset_traffic(update: Update, context: CallbackContext):
    """Reset traffic for the selected peer."""
    query = update.callback_query
    await query.answer()

    peer_name = query.data.replace("reset_traffic_", "")
    data = {"peerName": peer_name}  # Ensure the payload matches the API requirements

    response = await api_request("api/reset-traffic", method="POST", data=data)

    if "error" in response:
        await query.message.reply_text(f"❌ Error resetting traffic: {response['error']}")
    else:
        await query.message.reply_text(
            f"✅ Traffic for peer `{peer_name}` has been reset successfully!",
            parse_mode="Markdown"
        )

    # Go back to the reset traffic prompt for selecting another peer
    await reset_traffic_prompt(update, context)
    return SELECT_PEER  # Ensure the conversation state is correctly set for repeated actions



async def reset_expiry_prompt(update: Update, context: CallbackContext):
    """Prompt the user to select a peer for resetting expiry."""
    query = update.callback_query
    await query.answer()

    # Set the selected interface (this should be dynamic in case of multiple interfaces)
    context.user_data["selected_interface"] = "wg0.conf"  # Replace with appropriate logic if needed
    ...
    # Show the first page of peers
    await show_peer_page_expiry(update, context)
    return SELECT_PEER



async def show_peer_page_expiry(update: Update, context: CallbackContext):
    """Show a page of peers for resetting expiry with options to navigate or search."""
    peers = context.user_data.get("peers", [])
    page = context.user_data.get("peer_page", 0)
    page_size = 5
    start = page * page_size
    ...
    # Create buttons for peers on the current page
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"reset_expiry_{peer['peer_name']}")] for peer in peers[start:end]]

    # Add navigation buttons
    if start > 0:
        keyboard.append([InlineKeyboardButton("⬅️ Previous", callback_data="peer_page_previous_expiry")])
    if end < len(peers):
        keyboard.append([InlineKeyboardButton("➡️ Next", callback_data="peer_page_next_expiry")])

    # Add a search option and back button
    keyboard.append([InlineKeyboardButton("🔍 Search Peer by Name", callback_data="search_peer_name_expiry")])
    keyboard.append([InlineKeyboardButton("🔙 Back to Peers Menu", callback_data="peers_menu")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    # Compare with current message content and edit only if different
    query = update.callback_query
    current_message = query.message.text if query and query.message else ""
    new_message = "Select a peer to reset expiry:"

    if current_message != new_message:
        await query.edit_message_text(new_message, reply_markup=reply_markup)


async def handle_peer_pagination_expiry(update: Update, context: CallbackContext):
    """Handle pagination buttons for resetting expiry."""
    query = update.callback_query
    await query.answer()

    if query.data == "peer_page_previous_expiry":
        context.user_data["peer_page"] -= 1
    elif query.data == "peer_page_next_expiry":
        context.user_data["peer_page"] += 1

    await show_peer_page_expiry(update, context)

async def search_peer_by_name_expiry(update: Update, context: CallbackContext):
    """Prompt the user to enter a peer name for searching."""
    query = update.callback_query
    await query.answer()
    await query.message.reply_text("Enter the **name of the peer** to search for:", parse_mode="Markdown")
    return SEARCH_PEER

async def filter_peers_by_name_expiry(update: Update, context: CallbackContext):
    """Filter peers by the user's input for resetting expiry."""
    peer_name = update.message.text.strip()
    selected_interface = context.user_data.get("selected_interface")  # Retrieve the interface
    ...
    # Update pagination state with filtered peers
    context.user_data["peers"] = matched_peers
    context.user_data["peer_page"] = 0
...
async def reset_expiry(update: Update, context: CallbackContext):
    """Reset expiry for the selected peer."""
    query = update.callback_query
    await query.answer()

    peer_name = query.data.replace("reset_expiry_", "")
    data = {"peerName": peer_name}  # Ensure the payload matches the API requirements

    response = await api_request("api/reset-expiry", method="POST", data=data)

    if "error" in response:
        await query.message.reply_text(f"❌ Error resetting expiry: {response['error']}")
    else:
        await query.message.reply_text(
            f"✅ Expiry for peer `{peer_name}` has been reset successfully!",
            parse_mode="Markdown"
        )

    # Go back to the reset expiry prompt for selecting another peer
    await reset_expiry_prompt(update, context)
    return SELECT_PEER
...
async def edit_select_interface(update: Update, context: CallbackContext):
    ...
    context.user_data["selected_interface"] = selected_interface  # Ensure this is set
    ...
    # Save peers and initialize pagination
    context.user_data["peers"] = peers
    context.user_data["peer_page"] = 0
...
async def show_peer_page(update: Update, context: CallbackContext):
    """Show a paginated list of peers."""
    peers = context.user_data.get("peers", [])
    page = context.user_data.get("peer_page", 0)
    page_size = 5
    start = page * page_size
    ...
    # Create buttons for peers on the current page
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"edit_{peer['peer_name']}")] for peer in peers[start:end]]

    # Add navigation buttons
    if start > 0:
        keyboard.append([InlineKeyboardButton("⬅️ Previous", callback_data="peer_page_previous")])
    if end < len(peers):
        keyboard.append([InlineKeyboardButton("➡️ Next", callback_data="peer_page_next")])

    # Add search and back buttons
    keyboard.append([InlineKeyboardButton("🔍 Search Peer by Name", callback_data="search_peer_name")])
    keyboard.append([InlineKeyboardButton("🔙 Back to Interface Selection", callback_data="edit_peer")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    # Edit the message if this is a callback query update
    query = update.callback_query
    if query:
        await query.edit_message_text("Select a peer to edit or search for a peer by name:", reply_markup=reply_markup)
    else:
        await update.message.reply_text("Select a peer to edit or search for a peer by name:", reply_markup=reply_markup)

async def handle_peer_pagination(update: Update, context: CallbackContext):
    """Handle pagination buttons for peer selection."""
    query = update.callback_query
    await query.answer()

    # Adjust the current page
    if query.data == "peer_page_previous":
        context.user_data["peer_page"] -= 1
    elif query.data == "peer_page_next":
        context.user_data["peer_page"] += 1

    # Show the updated page
    await show_peer_page(update, context)
...
async def save_peer_changes(update: Update, context: CallbackContext):
    ...
    if "error" in response:
        ...
        # Add a back button to navigate to the peer menu
        keyboard = [[InlineKeyboardButton("🔙 Back to Peer Menu", callback_data="peers_menu")]]
        reply_markup = InlineKeyboardMarkup(keyboard)

        await update.message.reply_text(
            f"✅ Peer `{peer_name}` updated successfully!",
            reply_markup=reply_markup,
            parse_mode="Markdown"
        )
...
def main():
    ...
    states={
        ...
        SELECT_PEER_OR_SEARCH: [
            CallbackQueryHandler(handle_peer_pagination, pattern="peer_page_.*"),
    ...
    reset_traffic_conversation = ConversationHandler(
    entry_points=[CallbackQueryHandler(reset_traffic_prompt, pattern="reset_traffic")],
    states={
        SELECT_PEER: [
            CallbackQueryHandler(handle_peer_pagination_traffic, pattern="peer_page_.*_traffic"),
            CallbackQueryHandler(reset_traffic, pattern="reset_traffic_.*"),
            CallbackQueryHandler(search_peer_by_name_traffic, pattern="search_peer_name_traffic"),
        ],
        SEARCH_PEER: [MessageHandler(filters.TEXT & ~filters.COMMAND, filter_peers_by_name_traffic)],
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
    allow_reentry=True,
)
    ...
    reset_expiry_conversation = ConversationHandler(
    entry_points=[CallbackQueryHandler(reset_expiry_prompt, pattern="reset_expiry")],
    states={
        SELECT_PEER: [
            CallbackQueryHandler(handle_peer_pagination_expiry, pattern="peer_page_.*_expiry"),
            CallbackQueryHandler(reset_expiry, pattern="reset_expiry_.*"),
            CallbackQueryHandler(search_peer_by_name_expiry, pattern="search_peer_name_expiry"),
        ],
        SEARCH_PEER: [MessageHandler(filters.TEXT & ~filters.COMMAND, filter_peers_by_name_expiry)],
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
    allow_reentry=True,
)
    ...
    application.add_handler(reset_traffic_conversation)
    application.add_handler(reset_expiry_conversation)
