...
async def delete_peer_start(update: Update, context: CallbackContext):
    """Start the peer deletion process."""
    chat_id = update.effective_chat.id
    await context.bot.send_message(
        chat_id,
        text="Enter the **Peer Name** to delete:",
        parse_mode="Markdown"
    )
    return SELECT_PEER_NAME

async def delete_peer_get_interfaces(update: Update, context: CallbackContext):
    """Fetch available interfaces after entering a peer name."""
...
async def delete_peer_fetch(update: Update, context: CallbackContext):
    """Fetch peers by interface and display matching peers."""
    query = update.callback_query
    await query.answer()
    ...
    matching_peers = [peer for peer in peers if peer_name.lower() in peer["peer_name"].lower()]

    if not matching_peers:
        await query.message.reply_text(f"No peers found matching: **{peer_name}** on `{selected_interface}`.")
        return ConversationHandler.END

    # Display details for matching peers with delete buttons
    for peer in matching_peers:
        ...
        keyboard = [
            [InlineKeyboardButton("❌ Delete", callback_data=f"confirm_delete_{peer['public_key']}")],
        ]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def delete_peer_confirm(update: Update, context: CallbackContext):
    """Handle peer deletion confirmation."""
    query = update.callback_query
    await query.answer()
    ...
    response = await api_request(
        f"api/delete-peer",
        method="POST",
        data={"publicKey": public_key, "interface": selected_interface}
    )
    if "error" in response:
        await query.message.reply_text(f"❌ Error deleting peer: {response['error']}")
        return ConversationHandler.END

    await query.message.reply_text("✅ Peer deleted successfully!")
    return ConversationHandler.END

# Conversation Handler for Delete Peer
peer_delete_conversation = ConversationHandler(
    entry_points=[CallbackQueryHandler(delete_peer_start, pattern="delete_peer")],
    states={
        SELECT_PEER_NAME: [MessageHandler(filters.TEXT & ~filters.COMMAND, delete_peer_get_interfaces)],
        SELECT_INTERFACE: [CallbackQueryHandler(delete_peer_fetch, pattern="delete_interface_.*")],
        ConversationHandler.END: [CallbackQueryHandler(delete_peer_confirm, pattern="confirm_delete_.*")],
    },
...
def main():
    ...
    application.add_handler(peer_delete_conversation)
