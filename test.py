...
# Enable detailed state transition logs
ConversationHandler.LOGGING_LEVEL = logging.DEBUG
...
async def handle_create_interface_selection(update: Update, context: CallbackContext):
    ...
    # Extract the selected interface
...
async def validate_peer_name(update: Update, context: CallbackContext):
    peer_name = update.message.text
    logging.info(f"Validating peer name: {peer_name}")

    if not peer_name or not re.match(r"^[a-zA-Z0-9_]+$", peer_name):
        logging.warning(f"Invalid peer name entered: {peer_name}")
...
def main():
    ...
    states={
        SELECT_CREATE_INTERFACE: [CallbackQueryHandler(handle_interface_selection, pattern="interface_select_.*")],
        ...
        DNS: [CallbackQueryHandler(handle_first_usage, pattern="first_usage_.*")],
