...
async def peers_menu(update: Update, context: CallbackContext):
    ...
    keyboard = [
        ...
        [InlineKeyboardButton("⬇️ Download Config", callback_data="download_peer_config")],
        [InlineKeyboardButton("📷 Generate QR Code", callback_data="generate_peer_qr")],
        [InlineKeyboardButton("🔙 Back to Main Menu", callback_data="main_menu")],
    ]
    reply_markup = InlineKeyboardMarkup(keyboard)
    await context.bot.send_message(chat_id=chat_id, text=message, reply_markup=reply_markup, parse_mode="Markdown")
...
async def send_peer_qr(update: Update, context: CallbackContext):
    peer_name = context.args[0]  # Peer name from command args
    config = context.args[1]  # Config name from command args

    url = f"{API_BASE_URL}/api/export-peer-qr?peerName={peer_name}&config={config}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers={"Authorization": f"Bearer {API_KEY}"}) as response:
            if response.status == 200:
                ...
                await update.message.reply_text(f"❌ Error: {error.get('error', 'Failed to fetch QR code')}")

async def send_peer_config(update: Update, context: CallbackContext):
    peer_name = context.args[0]
    config = context.args[1]

    url = f"{API_BASE_URL}/api/export-peer?peerName={peer_name}&config={config}"
    async with aiohttp.ClientSession() as session:
        async with session.get(url, headers={"Authorization": f"Bearer {API_KEY}"}) as response:
            if response.status == 200:
                ...
                await context.bot.send_message(
                    chat_id=update.effective_chat.id,
                    text=f"Configuration for peer `{peer_name}`:\n\n```\n{peer_config}\n```",
                    parse_mode="Markdown"
                )
            else:
                error = await response.json()
...
async def download_peer_config(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text(
        "Enter the **Peer Name** and **Config Name** to download the configuration in the format:\n\n"
        "`peer_name config_name`",
        parse_mode="Markdown"
    )
...
async def generate_peer_qr(update: Update, context: CallbackContext):
    query = update.callback_query
    await query.answer()
    await query.message.reply_text(
        "Enter the **Peer Name** and **Config Name** to generate a QR code in the format:\n\n"
        "`peer_name config_name`",
        parse_mode="Markdown"
    )
...
async def handle_peer_qr_generation(update: Update, context: CallbackContext):
    try:
        peer_name, config_name = update.message.text.split()
        url = f"{API_BASE_URL}/api/export-peer-qr?peerName={peer_name}&config={config_name}"
        async with aiohttp.ClientSession() as session:
            async with session.get(url, headers={"Authorization": f"Bearer {API_KEY}"}) as response:
                if response.status == 200:
                    image = await response.read()
...
def main():
    ...
    application.add_handler(CommandHandler("peer_qr", send_peer_qr))
    application.add_handler(CommandHandler("peer_config", send_peer_config))
    application.add_handler(CallbackQueryHandler(download_peer_config, pattern="download_peer_config"))
    application.add_handler(CallbackQueryHandler(generate_peer_qr, pattern="generate_peer_qr"))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_peer_config_download))
    application.add_handler(MessageHandler(filters.TEXT & ~filters.COMMAND, handle_peer_qr_generation))
