...
async def backup_menu(update: Update, context: CallbackContext):
    ...
    message = (
        "🗂️ **Backup Management**\n\n"
        "Choose an option below to manage backups:"
    )
    keyboard = [
...
async def restore_backup(update: Update, context: CallbackContext):
    ...
    message = "🗂️ **Available Backups for Restoration**\n\n"
    for idx, backup in enumerate(backups, start=1):
        message += f"{idx}. `{backup}`\n"

    message += "\nPlease send the **number** of the backup you want to restore:"
    context.user_data["backups"] = backups
...
def main():
    ...
    entry_points=[CallbackQueryHandler(backup_menu, pattern="backup_menu")],
    states={
