...
SELECT_PEER_OR_SEARCH, SEARCH_PEER, SELECT_PEER_TO_EDIT, EDIT_OPTION, SET_DATA_LIMIT, SET_DNS, SET_EXPIRY_TIME = range(6)
...
async def edit_select_interface(update: Update, context: CallbackContext):
    ...
    # Add a button to allow manual search
    keyboard.append([InlineKeyboardButton("🔍 Search for Peer by Name", callback_data="search_peer_name")])
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.message.reply_text("Select a peer to edit or search by name:", reply_markup=reply_markup)
...
async def select_peer_to_edit(update: Update, context: CallbackContext):
    ...
    selected_interface = context.user_data.get("selected_interface", "unknown")
    keyboard = [
    ...
    reply_markup = InlineKeyboardMarkup(keyboard)
    await query.message.reply_text(
        f"Editing options for peer: **{selected_peer}** on interface: **{selected_interface}**",
        reply_markup=reply_markup,
        parse_mode="Markdown"
    )
    return EDIT_OPTION
