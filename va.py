...
async def show_peer_page_expiry(update: Update, context: CallbackContext):
    ...
    query = update.callback_query
    if query:
        await query.edit_message_text("Select a peer to reset expiry:", reply_markup=reply_markup)
    else:
        await update.message.reply_text("Select a peer to reset expiry:", reply_markup=reply_markup)
