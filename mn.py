...
async def edit_data_limit(update: Update, context: CallbackContext):
    ...
    # Fetch the current data limit from user_data
    current_data_limit = context.user_data.get("selected_peer_data", {}).get("limit_human", "Unknown")
...
async def edit_expiry_time(update: Update, context: CallbackContext):
    ...
    # Fetch the current expiry time from user_data
    current_expiry = context.user_data.get("selected_peer_data", {}).get("expiry_time", {})
    current_expiry_days = current_expiry.get("days", 0)
    current_expiry_months = current_expiry.get("months", 0)
    current_expiry_hours = current_expiry.get("hours", 0)
    current_expiry_minutes = current_expiry.get("minutes", 0)

    await update.callback_query.message.reply_text(
        f"Current Expiry Time: **{current_expiry_days} days, {current_expiry_months} months, "
        f"{current_expiry_hours} hours, {current_expiry_minutes} minutes**\n"
