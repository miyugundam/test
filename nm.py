...
async def reset_traffic(update: Update, context: CallbackContext):
    ...
    if "error" in response:
        ...
        await query.message.reply_text(f"✅ Traffic for peer `{peer_name}` has been reset successfully!", parse_mode="Markdown")
...
async def reset_expiry_prompt(update: Update, context: CallbackContext):
    ...
    # Create buttons for peers
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"reset_expiry_{peer['peer_name']}")] for peer in peers]
    keyboard.append([InlineKeyboardButton("🔙 Back to Peers Menu", callback_data="peers_menu")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.message.reply_text("Select a peer to reset expiry:", reply_markup=reply_markup)
    return TOGGLE_BLOCK
...
async def reset_expiry(update: Update, context: CallbackContext):
    ...
    response = await api_request(f"api/reset-expiry?peerName={peer_name}", method="POST")

    if "error" in response:
        await query.message.reply_text(f"❌ Error resetting expiry: {response['error']}")
    else:
        await query.message.reply_text(f"✅ Expiry for peer `{peer_name}` has been reset successfully!", parse_mode="Markdown")
...
def main():
    ...
    states={
        TOGGLE_BLOCK: [CallbackQueryHandler(reset_expiry, pattern="reset_expiry_.*")]
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
)
