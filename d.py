...
async def handle_interface_selection(update: Update, context: CallbackContext):
    ...
    # Fetch peers with the given name for the selected interface
    ...
    for peer in peers:
        ...
        keyboard = [[InlineKeyboardButton("❌ Delete Peer", callback_data=f"delete_{peer['peer_name']}")]]
        reply_markup = InlineKeyboardMarkup(keyboard)
...
async def handle_peer_deletion(update: Update, context: CallbackContext):
    ...
    peer_name = query.data.replace("delete_", "")
...
def main():
    ...
    application.add_handler(CallbackQueryHandler(handle_peer_deletion, pattern="^delete_.*$"))
