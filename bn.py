...
SELECT_INTERFACE, SELECT_PEER_OR_SEARCH, SEARCH_PEER, SELECT_PEER_TO_EDIT, EDIT_OPTION, SET_DATA_LIMIT, SET_DNS, SET_EXPIRY_TIME = range(8)
...
async def reset_traffic_prompt(update: Update, context: CallbackContext):
    """Prompt the user to select a peer for resetting traffic with pagination and search."""
    query = update.callback_query
    await query.answer()
    ...
    return SELECT_PEER  # Ensure the conversation state is correctly set


async def show_peer_page(update: Update, context: CallbackContext):
    """Show a page of peers with options to navigate or search."""
    ...
    # Edit the message if it's an update, otherwise send a new one
    query = update.callback_query
    if query:
        await query.edit_message_text("Select a peer to reset traffic:", reply_markup=reply_markup)
    else:
        await update.message.reply_text("Select a peer to reset traffic:", reply_markup=reply_markup)
...
async def filter_peers_by_name(update: Update, context: CallbackContext):
    """Filter peers by the user's input."""
    search_query = update.message.text.strip().lower()
    all_peers = context.user_data.get("peers", [])

    matched_peers = [peer for peer in all_peers if search_query in peer["peer_name"].lower()]
    if not matched_peers:
        await update.message.reply_text("❌ No peers found matching your search. Try again.")
        return SEARCH_PEER

    # Update the context with matched peers and reset pagination
...
async def reset_expiry_prompt(update: Update, context: CallbackContext):
    """Prompt the user to select a peer for resetting expiry with pagination and search."""
    query = update.callback_query
    await query.answer()
...
def main():
    ...
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
    allow_reentry=True,  # Ensure the conversation can be re-entered
)
