...
async def reset_traffic_prompt(update: Update, context: CallbackContext):
    ...
    keyboard = [[InlineKeyboardButton(peer["peer_name"], callback_data=f"reset_traffic_{peer['peer_name']}")] for peer in peers]
    keyboard.append([InlineKeyboardButton("🔙 Back to Peers Menu", callback_data="peers_menu")])
    reply_markup = InlineKeyboardMarkup(keyboard)

    await query.message.reply_text("Select a peer to reset traffic:", reply_markup=reply_markup)
    return TOGGLE_BLOCK
...
async def reset_traffic(update: Update, context: CallbackContext):
    ...
    response = await api_request(f"api/reset-traffic?peerName={peer_name}", method="POST")
...
def main():
    ...
    states={
        TOGGLE_BLOCK: [CallbackQueryHandler(reset_traffic, pattern="reset_traffic_.*")]
    },
    fallbacks=[CallbackQueryHandler(peers_menu, pattern="peers_menu")],
)
